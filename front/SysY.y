...
MulExp
    : UnaryExp  { $$ = $1; }
    | UnaryExp '*'  MulExp  { $$ = new arith_exp_node(MUL, $1, $3); }
    | UnaryExp '/'  MulExp  { $$ = new arith_exp_node(DIV, $1, $3); }
    | UnaryExp '%'  MulExp  { $$ = new arith_exp_node(MOD, $1, $3); }
AddExp
    : MulExp   { $$ = $1; }
    | MulExp '+' AddExp { $$ = new arith_exp_node(ADD, $1, $3); }
    | MulExp '-' AddExp { $$ = new arith_exp_node(SUB, $1, $3); }
RelExp
    : AddExp   { $$ = $1; }
    | AddExp '<' RelExp { $$ = new arith_exp_node(L, $1, $3); }
    | AddExp '>' RelExp { $$ = new arith_exp_node(G, $1, $3); }
    | AddExp T_GE RelExp { $$ = new arith_exp_node(GE, $1, $3); }
    | AddExp T_LE RelExp { $$ = new arith_exp_node(LE, $1, $3); }
EqExp
    : RelExp   { $$ = $1; }
    | RelExp T_EQ EqExp  { $$ = new arith_exp_node(EQ, $1, $3); }
    | RelExp T_NE EqExp  { $$ = new arith_exp_node(NE, $1, $3); }
LAndExp
    : EqExp     { if( $1->exp_type == EXP_PTR) 
                      yyerror("pointer should not be in expression");
                  $$ = new cond_exp_node(NONE, $1, nullptr); }
    | EqExp '&' LAndExp { $$ = new cond_exp_node(AND, $1, $3); }
LOrExp
    : LAndExp   { if( $1->exp_type == EXP_PTR) 
                      yyerror("pointer should not be in expression");
                  $$ = $1; }
    | LAndExp '|' LOrExp { $$ = new cond_exp_node(OR, $1, $3); }
